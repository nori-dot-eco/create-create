...
async function run({
  ...
  const { help, t, template } = options;
  if (help) return;

  log();
  log(
    `Welcome to ${pico.bold(
      pico.blue('@nori-dot-com/create')
    )} – the quickest way to get started with Nori!`
  );
  log();

  const __dirname = fileURLToPath(new URL('.', import.meta.url));
  const templatesPath = path.join(__dirname, '..', 'templates');
  let templateId = template || t;
  ...
  if (!templateId || !frameworkId) {
    ...
    if (frameworkTemplates == undefined) return;
  ...
  if (frameworkMeta == undefined) return;

  // Get template meta
  const templateMeta = templates.find(({ id }) => id === templateId);
  if (templateMeta == undefined) return;
  ...
  log(`Creating a new Nori app in ${pico.green(targetPath)}.`);
  log();
  log(
  ...
  if (!skipGit) {
    ...
    await execa(
      ...
      [
        ...
        '--no-verify',
        '--message',
        'Initial commit from @nori-dot-com/create',
      ],
      { cwd: targetPath }
    );
...
(async () => {
  ...
  try {
    ...
    log(pico.red((<Error>error).message));
    process.exit(1);
  }
